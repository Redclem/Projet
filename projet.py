...
class Fenetre(Tk):
    ...
    def clic_canvas(self, event):
        ...
        if self.etatTableau[x][y] == 0:
            self.etatTableau[x][y] = CROIX

            if self.test_victoire():
                self.label.config(text=NOMS[CROIX] + " a gagné!")
                ...
                self.joueur = ROND
                self.label.config(text=NOMS[ROND] + " joue")
    ...
    def bot_joue(self):
        ...
        while self.etatTableau[x][y] != 0:
            x = randint(0,2)
            y = randint(0,2)

        btime = time()

        self.etatTableau[x][y] = ROND

        if self.test_victoire():
            self.label.config(text=NOMS[ROND] + " a gagné!")
            self.victoire = True
            ...
            self.joueur = CROIX
            self.label.config(text=NOMS[CROIX] + " joue")
